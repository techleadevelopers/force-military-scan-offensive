...
class SniperDecisionEngine:
    ...
    async def execute(self, findings: List, scan_events: List[Dict] = None,
        ...
        self.log(
            "[SNIPER DECISION] ★ APT Level 5 Decision Engine ACTIVATED — "
        ...
        self.log(
            f"[SNIPER DECISION] ★ Engine complete in {elapsed}s — "
    ...
    def _phase_intelligence(self, headers: Dict = None,
                            ...
                            error_samples: List[str] = None) -> Dict:
        self.log(
            "[DECISION §1] DEEP FINGERPRINTING — Extracting exact versions + CVE mapping...",
        ...
        if cves:
            self.log(
                f"[DECISION §1] {fp_report['components_detected']} components, "
            ...
            for cve in cves[:3]:
                self.log(
                    f"  └─ {cve['id']}: {cve['component']} {cve.get('version', '?')} "
                    f"(CVSS {cve['cvss']}) — {cve['description'][:60]}",
                    "error", "decision_engine"
                )
        else:
            self.log(
                f"[DECISION §1] {fp_report['components_detected']} components detected, no CVE matches",
    ...
    def _phase_prediction(self) -> Dict:
        self.log(
            "[DECISION §2] PREDICTIVE ANALYSIS — Predicting attack surface before scanning...",
            "warn", "decision_engine"
        )
        ...
        self.log(
            f"[DECISION §2] {pred_report['total_predictions']} vulnerabilities predicted "
        ...
        for pred in prioritized[:3]:
            self.log(
                f"  └─ {pred['vuln']}: {pred.get('confidence', 0):.0%} confidence "
    ...
    def _phase_temporal(self, findings: List, scan_events: List[Dict] = None) -> Dict:
        self.log(
            "[DECISION §3] TEMPORAL CORRELATION — Analyzing event sequences across time...",
            "warn", "decision_engine"
        )
        ...
        if patterns:
            for p in patterns[:3]:
                self.log(
                    f"  └─ Pattern: {p['pattern']} → {p['decision']} "
                    f"(confidence: {p['confidence']:.0%})",
                    "warn", "decision_engine"
                )
        else:
            self.log(
                "[DECISION §3] No temporal patterns detected (insufficient event history)",
                "info", "decision_engine"
            )
    ...
    def _phase_bayesian(self, findings: List) -> Dict:
        self.log(
            "[DECISION §4] BAYESIAN INFERENCE — Calculating success probabilities...",
            "warn", "decision_engine"
        )
        ...
        self.log(
            f"[DECISION §4] {bayes_report['total_evaluated']} vectors evaluated — "
        ...
        for top in bayes_report.get("top_attack_targets", [])[:3]:
            self.log(
                f"  └─ {top['vector']}: P(success)={top['probability']:.0%} → ATTACK",
                "error", "decision_engine"
            )
    ...
    def _phase_dynamic_chains(self, findings: List) -> Dict:
        self.log(
            "[DECISION §5] DYNAMIC CHAIN CONSTRUCTION — Building adaptive exploit paths...",
            "warn", "decision_engine"
        )
        ...
        self.log(
            f"[DECISION §5] Chain built: {chain_report['chain_length']} steps, "
        ...
        for step in chain[:3]:
            self.log(
                f"  └─ Step {step['step']}: {step['technique']} "
                f"(P={step['probability']:.0%}) → {', '.join(step['leads_to'][:3])}",
                "info", "decision_engine"
            )
    ...
    def _phase_genetic_evolution(self) -> Dict:
        self.log(
            "[DECISION §6] GENETIC PAYLOAD EVOLUTION — Initializing population + fitness evaluation...",
            "warn", "decision_engine"
        )
        ...
        self.log(
            f"[DECISION §6] Generation {gen_report['current_generation']}: "
    ...
    def _phase_exfiltration_triage(self, findings: List) -> Dict:
        self.log(
            "[DECISION §7] SMART EXFILTRATION TRIAGE — Classifying data value + prioritization...",
            "warn", "decision_engine"
        )
        ...
        if data_items:
            ...
            self.log(
                f"[DECISION §7] {exfil_report['total_items']} items triaged: "
            ...
            if regulatory:
                ...
                self.log(f"  └─ Regulatory exposure: {regs}", "error", "decision_engine")
        else:
            exfil_report = {"total_items": 0, "immediate": 0, "queued": 0, "deferred": 0}
            self.log("[DECISION §7] No data items to triage", "info", "decision_engine")

        return exfil_report

    def _phase_anti_forensics(self, scan_events: List[Dict] = None) -> Dict:
        self.log(
            "[DECISION §8] ANTI-FORENSICS ASSESSMENT — Evaluating stealth posture...",
            "warn", "decision_engine"
        )
        ...
        self.log(
            f"[DECISION §8] Stealth score: {assessment['stealth_score']:.0%}, "
        ...
        if assessment["detected_count"] > 0:
            self.log(
                f"  └─ {assessment['detected_count']} detection indicators triggered",
                "error", "decision_engine"
            )
            for rec in recommendations[:2]:
                self.log(
                    f"  └─ Recommend: {rec['technique']} "
    ...
    def _phase_multi_objective(self, findings: List) -> Dict:
        self.log(
            "[DECISION §9] MULTI-OBJECTIVE OPTIMIZATION — Pareto frontier analysis...",
            "warn", "decision_engine"
        )
        ...
        if possible_actions:
            ...
            self.log(
                f"[DECISION §9] {mo_report['total_evaluated']} actions evaluated — "
            ...
            self.log("[DECISION §9] No actions to optimize", "info", "decision_engine")
