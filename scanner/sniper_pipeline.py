...
class SniperPipeline:
    ...
    async def _phase_1_ingest(self):
        ...
        pipeline_log("[PHASE 1/6] I Launching full orchestrator scan...", "warn", "ingest")
    ...
    async def _phase_risk_score(self):
        ...
        if mode == "AUTO_DUMP":
            if override:
                pipeline_log(
                    f"[AUTO-DUMP] â˜…â˜… {override}  score forced to {score:.4f} ({total} findings, {crit_confirmed} critical confirmed)",
                    "error", "risk_score"
                )
            else:
                pipeline_log(
                    f"[AUTO-DUMP] â˜… RISK SCORE {score:.4f} > 0.85  AUTO-DUMP TRIGGERED ({total} findings)",
                    "error", "risk_score"
                )
            pipeline_log(
    ...
    async def _phase_2f_incident_absorber(self):
        ...
        if cost_reward_matrix:
            ...
            for entry in top_roi:
                pipeline_log(
                    f"[COST-REWARD] {entry['vector'][:50]}  Cost: ${entry['attack_cost_usd']} â†’ Loss: ${entry['potential_loss_usd']:,.0f} (ROI: {entry['roi_multiplier']}x)",
                    "error", "incident_absorber"
                )
